...
/*   Updated: 2024/07/17 01:40:48 by aderraj          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_wall(t_game *game, int i, int j)
{
	...
	if (j > 0 && j <= game->map_width - 1 && game->map[i][j - 1] == '1')
		ft_cpy_img(game->wall, game->render_img, (j * TILE_SIZE) - 32, i
			* TILE_SIZE);
	ft_cpy_img(game->wall, game->render_img, j * TILE_SIZE, i * TILE_SIZE);
}
...
void	render_exit(t_game *game)
{
	...
	{
		ft_cpy_img(game->exit[frame], game->render_img, game->exit_x
			* TILE_SIZE, game->exit_y * TILE_SIZE);
		if (delay < 1)
			delay++;
		else
		{
		...
		ft_cpy_img(game->exit[8], game->render_img, game->exit_x * TILE_SIZE,
			game->exit_y * TILE_SIZE);
	else
		ft_cpy_img(game->exit[0], game->render_img, game->exit_x * TILE_SIZE,
			game->exit_y * TILE_SIZE);
}
...
void	render_counter(t_game *game)
{
	...
	while (i < 15)
	{
		...
		while (j < 30)
		{
			mlx_pixel_put(game->mlx, game->win, j, i, 0x00000000);
			j++;
		}
		i++;
	}
