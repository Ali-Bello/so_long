...
t_sprite	*new_sprite(char *filename, void *mlx, void *win)
{
	...
	if (!s->img)
		return (free(s), NULL);
	s->img->img_ptr = mlx_xpm_file_to_image(mlx, filename, &s->img->width,
			&s->img->height);
	if (!s->img->img_ptr)
		return (free(s->img), free(s), NULL);
	s->img->img_data = mlx_get_data_addr(s->img->img_ptr, &s->img->bpp,
...
{
	return (*(unsigned int *)((img->img_data + (y * img->line_len) + (x
					* img->bpp / 8))));
}
...
t_img	*new_img(t_sprite *sprite)
{
	...
	img->img_data = mlx_get_data_addr(img->img_ptr, &img->bpp, &img->line_len,
			&img->endian);
	img->width = IMG_WIDTH;
...
t_frame	*new_frame(t_sprite *sprite)
{
	...
	cpy_img(sprite->img, frame->img, sprite->animations->x,
		sprite->animations->y);
	frame->next = NULL;
	return (frame);
}
