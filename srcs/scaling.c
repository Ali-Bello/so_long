...
void scale_image(t_mlx *mlx, t_img *img, t_scale *data)
{
	...
	img->scaled_img = (char *)mlx_get_data_addr(tmp, &data->bpp, &data->line_len, &(img->endian));
    ...
    while (i < data->scaled_width)
	{
        ...
        while (j < data->scaled_height)
		{
            ...
            data->orig_idx = data->orig_y * img->line_len + data->orig_x * (img->bpp / 8);
            data->scaled_idx = j * data->scaled_width * (img->bpp / 8) + i * (img->bpp / 8);
            my_mlx_pixel_put(img, data, i, j, *((int *)(img->img_data + data->orig_idx)));
			j++;
        }
		i++;
    }
